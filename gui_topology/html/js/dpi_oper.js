...
dpi_oper.prototype.dpiInfo2HostData = function() {
    ...
    for(var x in this.dpi_info['known.flows']) {
        ...
        if(this.checkIPv4(entry['host_a.name']) && this.checkIPv4(entry['host_b.name'])) {
            ...
            if(! this.global_data['proto_traffic'].hasOwnProperty(protocol)) {
                this.global_data['proto_traffic'][protocol] = 0;
            }
            ...
            var last_value = this.global_data['proto_traffic'][protocol];
            this.global_data['proto_traffic'][protocol] = 0.875*last_value + 0.125*parseInt(bytes);
        }
...
dpi_oper.prototype.updatePortTable = function(data) {
    ...
    for(var x in port_info) {
        ...
        if(hostName) {
            ...
            if(!this.global_data['host_traffic'].hasOwnProperty(hostName)) {
                this.global_data['host_traffic'][hostName] = 0;
            }
            var last_value = this.global_data['host_traffic'][hostName];
            this.global_data['host_traffic'][hostName] = 0.875*last_value + 0.125*parseInt(tx_byte);
        }
    }
...
dpi_oper.prototype.init_global_data = function() {
    this.global_data['sw_num'] = 3; // hard code
    this.global_data['host_num'] = 6; // hard code
    this.global_data['host_traffic'] = {}; // record hosts' EWMA tx byte
    this.global_data['proto_traffic'] = {}; // record proto EWMA byte
}
